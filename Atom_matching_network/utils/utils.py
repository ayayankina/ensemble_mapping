...
def get_acc_on_test(pred,data):
        """
    Вычисляет точность (accuracy) предсказаний на тестовых данных, 
    учитывая эквивалентные атомы (равноценные варианты ответов).

    Параметры:
    ----------
    pred : array-like
        Массив предсказанных значений (индексов атомов).
    data : объект
        Объект, содержащий информацию о тестовых данных, должен иметь атрибуты:
        - rp_mapper: массив эталонных (правильных) атомов
        - eq_as: список множеств эквивалентных атомов (групп взаимозаменяемых ответов)

    Возвращает:
    -----------
    float
        Значение точности (accuracy) в диапазоне [0, 1], где:
        1 - все предсказания верны (с учетом эквивалентных атомов)
        0 - все предсказания неверны

    Алгоритм:
    ---------
    1. Преобразует эталонные атомы в массив numpy
    2. Создает словарь для быстрого поиска группы эквивалентных атомов
    3. Для каждого элемента проверяет, принадлежит ли предсказание 
       в ту же группу эквивалентности, что и эталонный атом
    4. Подсчитывает долю правильных ответов
    """
