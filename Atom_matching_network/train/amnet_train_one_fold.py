...
parser.add_argument('--fold', type=int, default=0)  # Параметр для задания номера фолда

args = parser.parse_args()

# Путь к фолдам
folds_dir = "SanitizeMol_data/folds"

# Загружаем данные для текущего фолда
train_data = pd.read_csv(os.path.join(folds_dir, f"train_fold{args.fold}.csv"), names=['reactions', 'edits'])
valid_data = pd.read_csv(os.path.join(folds_dir, f"val_fold{args.fold}.csv"), names=['reactions', 'edits'])
...
path = f'experiment_folds_{args.fold}'
